// src/screens/content/EditContentScreen.tsx
...
type Props = {
  navigation: AppNavigationProp<'EditContent'>;
  route: AppRouteProp<'EditContent'>;
};
...
const EditContentScreen = ({ navigation, route }: Props) => {
  // Pre-populate with existing data — swap with Redux/API lookup by route.params.contentId
    ...
    <>
      ...
      <View style={styles.body}>
        ...
        >
          ...
          <Dropdown label="Select Platform" value={platform} onPress={() => setShowPlatformPicker(true)} />

          <Text style={styles.fieldLabel}>Type</Text>
          <Dropdown label="Select Content Type" value={contentType} onPress={() => setShowTypePicker(true)} />
      ...
      {showPlatformPicker && (
        <PickerOverlay options={PLATFORMS} onSelect={setPlatform} onClose={() => setShowPlatformPicker(false)} />
      )}
      {showTypePicker && (
        <PickerOverlay options={CONTENT_TYPES} onSelect={setContentType} onClose={() => setShowTypePicker(false)} />
      )}
    </>
  );
};
...
const styles = StyleSheet.create({
  ...
  uploadBtn: {
    ...
    marginBottom: verticalScale(14), borderWidth: 1, borderColor: Colors.borderStrong,
  },
