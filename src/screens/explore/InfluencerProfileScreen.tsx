...
function ProfileHeader({ navigation }: { navigation: any }) {
  return (
    <>
      <LinearGradient colors={['#0097B2', '#7ED957']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={styles.headerGradient}>
      ...
      <View style={styles.profileMetaRow}>
        <LinearGradient colors={[Colors.teal, Colors.lime]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.avatarGradient} />
...
function StatsDashboard({ profile }: { profile: ProfileData }) {
  return (
    <View style={styles.statsDashboardContainer}>
      <View style={styles.statBox}>
        <Text style={Typography.statNumber}>{profile.following}</Text>
        <Text style={[Typography.caption, { color: Colors.textMuted }]}>Following</Text>
      </View>
      <View style={styles.statBox}>
        <Text style={Typography.statNumber}>{profile.followers}</Text>
        <Text style={[Typography.caption, { color: Colors.textMuted }]}>Followers</Text>
      </View>
      <View style={styles.statBox}>
        <Text style={Typography.statNumber}>{profile.engagement}</Text>
        <Text style={[Typography.caption, { color: Colors.textMuted }]}>Engagement</Text>
      </View>
      <View style={styles.statBox}>
        <Text style={Typography.statNumber}>{profile.rating}</Text>
  ...
  return (
    <View style={styles.primaryActionRow}>
      <Pressable style={styles.actionBtnLeft}>
        <LinearGradient colors={[Colors.teal, Colors.lime]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={styles.gradientBtnBg}>
...
function SegmentedTabs({ activeTab, onTabChange }: { activeTab: TabType; onTabChange: (tab: TabType) => void }) {
  return (
    <View style={styles.segmentedTabContainer}>
      <Pressable style={[styles.segmentButton, activeTab === 'overview' && styles.segmentButtonActive]} onPress={() => onTabChange('overview')}>
      ...
      <Pressable style={[styles.segmentButton, activeTab === 'content' && styles.segmentButtonActive]} onPress={() => onTabChange('content')}>
        <Text style={[Typography.buttonSecondary, activeTab === 'content' ? { color: Colors.textPrimary } : { color: Colors.textSecondary }]}>Content</Text>
      </Pressable>
      <Pressable style={[styles.segmentButton, activeTab === 'pricing' && styles.segmentButtonActive]} onPress={() => onTabChange('pricing')}>
...
function OverviewTabContent({ direction }: { direction: any }) {
  return (
    <Animated.View entering={direction('overview').entering} exiting={direction('overview').exiting}>
      ...
      <View style={styles.metaBlockSectionCustomBorder}>
        ...
        <View style={styles.badgeWrapRow}>
          <View style={styles.pillBadgeContainerCustomBorder}><Text style={[Typography.bodySmall, { color: Colors.textPrimary }]}>#Blog</Text></View>
          <View style={styles.pillBadgeContainerCustomBorder}><Text style={[Typography.bodySmall, { color: Colors.textPrimary }]}>#Photo</Text></View>
          <View style={styles.pillBadgeContainerCustomBorder}><Text style={[Typography.bodySmall, { color: Colors.textPrimary }]}>#VideoStream</Text></View>
          <View style={styles.pillBadgeContainerCustomBorder}><Text style={[Typography.bodySmall, { color: Colors.textPrimary }]}>#Tutorial</Text></View>
        </View>
      </View>
      <View style={styles.metaBlockSectionCustomBorder}>
        <Text style={[Typography.h2, styles.sectionBlockHeading]}>Achivements</Text>
        <View style={styles.badgeWrapRow}>
          <View style={styles.pillBadgeContainerCustomBorder}><Text style={[Typography.bodySmall, { color: Colors.textPrimary }]}>🏆 Top Creator 2023</Text></View>
          <View style={styles.pillBadgeContainerCustomBorder}><Text style={[Typography.bodySmall, { color: Colors.textPrimary }]}>✅ Verified Influencer</Text></View>
        </View>
        <View style={[styles.badgeWrapRow, { marginTop: 8 }]}>
          <View style={styles.pillBadgeContainerCustomBorder}><Text style={[Typography.bodySmall, { color: Colors.textPrimary }]}>👥 100K+ Followers</Text></View>
          <View style={styles.pillBadgeContainerCustomBorder}><Text style={[Typography.bodySmall, { color: Colors.textPrimary }]}>🤝 Brand Partner</Text></View>
        </View>
      </View>
    </Animated.View>
  );
    ...
    <Animated.View entering={direction('content').entering} exiting={direction('content').exiting}>
      <View style={styles.contentGridSystem}>
        <View style={styles.contentColumnItem}>
          <ImageBackground source={{ uri: 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?q=80&w=500' }} style={styles.contentCardMediaFrame} imageStyle={styles.contentCardImageRadius}>
            <View style={styles.contentCardDimmingOverlay}><Text style={styles.contentOverlaySubtitleText}>You won't believe this is a website</Text></View>
          </ImageBackground>
        ...
        <View style={styles.contentColumnItem}>
          <ImageBackground source={{ uri: 'https://images.unsplash.com/photo-1517841905240-472988babdf9?q=80&w=500' }} style={styles.contentCardMediaFrame} imageStyle={styles.contentCardImageRadius}>
            <View style={styles.contentCardDimmingOverlay}><Text style={styles.contentOverlaySubtitleText}>Many of your apps are not working right now</Text></View>
          </ImageBackground>
          <Pressable style={styles.contentMediaActionFooter}>
        ...
        <View style={styles.contentColumnItem}>
          <ImageBackground source={{ uri: 'https://images.unsplash.com/photo-1539571696357-5a69c17a67c6?q=80&w=500' }} style={styles.contentCardMediaFrame} imageStyle={styles.contentCardImageRadius}>
            <View style={styles.contentCardDimmingOverlay}><Text style={styles.contentOverlaySubtitleText}>"Girls can't be good at coding, it's not their cup of tea."</Text></View>
          </ImageBackground>
          <Pressable style={styles.contentMediaActionFooter}>
...
function PricingTabContent({ direction, navigation, profile }: { direction: any; navigation: any; profile: ProfileData }) {
  return (
    <Animated.View entering={direction('pricing').entering} exiting={direction('pricing').exiting} style={styles.pricingDeckContainer}>
      {[
        ...
        <Pressable
          ...
          onPress={() => navigation.navigate('RequestQuote', { platformId: platform.id, influencerName: profile.name })}
        >
          <View style={styles.accordionLeftSection}>
            <Ionicons name={platform.icon} size={20} color={platform.color} style={styles.platformHeaderIcon} />
  ...
  return (
    <SafeAreaView edges={['top']} style={styles.rootContainer}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        <StatsDashboard profile={MOCK_PROFILE} />
        <ActionButtons />
        <SegmentedTabs activeTab={activeTab} onTabChange={handleTabChange} />
        {activeTab === 'overview' && <OverviewTabContent direction={tabDirection} />}
        {activeTab === 'content' && <ContentTabContent direction={tabDirection} profile={MOCK_PROFILE} />}
        {activeTab === 'pricing' && <PricingTabContent direction={tabDirection} navigation={navigation} profile={MOCK_PROFILE} />}
      </ScrollView>
    </SafeAreaView>
  )
}